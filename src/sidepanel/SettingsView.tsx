        ...
        <article className="sophia-card settings-hero-card">
          <div className="settings-hero-top" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div className={`add-model-dropdown${dropdownOpen ? " open" : ""}`} ref={dropdownRef}>
              <button className="add-model-trigger-icon" onClick={() => setDropdownOpen(!dropdownOpen)}>
          ...
          {showAddForm ? (
            <div className="model-config-form">
              ...
              <div style={{ display: "flex", gap: "var(--space-2)" }}>
                ...
                <button className="btn-primary" onClick={handleAddSave} style={{ flex: 1 }}>添加模型</button>
              </div>
            </div>
          ) : null}
          ...
          {settings.models.length > 0 ? (
                    ...
                    {editingId === model.id ? (
                      <div className="model-edit-form-wrapper is-open">
                        <div className="model-edit-form">
                          ...
                          {editErrorMsg ? (
                            <div style={{ fontSize: "var(--text-xs)", color: "var(--danger)", marginBottom: "var(--space-2)" }}>{editErrorMsg}</div>
                          ) : null}
                          <div style={{ display: "flex", gap: "var(--space-2)" }}>
                            <button className="btn-secondary" onClick={cancelEditForm}>取消</button>
                            <button className="btn-primary" onClick={handleEditSave} style={{ flex: 1 }}>保存修改</button>
                          </div>
                        </div>
                      </div>
                    ) : null}
            ...
            <div className="model-empty" style={{ padding: "var(--space-4) 0" }}>尚未配置模型，点击"+ 添加"开始配置</div>
          )}
        </article>
